...
module ParseWikipediaXML
  ...
  def self.new(command, argv)
      ParseWikipediaXMLCli.start(argv.unshift(command))
  end
  ...
  # Params:
  class ParseWikipediaXMLCli < Thor
    ...
    def parser
      if options[:japanese]
        JapParser.new(options)
      else
        EngParser.new(options)
      end
    end
